...
<!-- TODO task_config.json path needs checking. Where is it? -->
...
_**Note: don't modify config file location and key-words in .json**_
...
- Open the dropdown menu under config and click on add new config, then specify:
  - The _**serial port**_ of the specific microcontroller (e.g. on windows, COM1)
  - The _**baud rate**_, specified in the arduino sketch
  - List the _**inputs**_ that are read from the microcontroller, separated by commas. For example, this could be lever presses.
