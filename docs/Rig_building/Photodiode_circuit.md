...
:alt: Photodoide circuit with CJMCUM-101 board (Mathis lab)
...
:alt: Photodiode circuit with TSL257 photodiode (Tolias lab)
:class: bg-primary mb-1
:width: 400px
:align: center
```

To test the latency of the system run a session and then run the notebook found in the repo under this path: `/mouse_task/latency_tests/Latency_test_notebook/Latency_testing.ipynb` on the resulting `_PROC` file that gets saved by the DLClive-GUI. You may need to adjust the `flip_photodiode` and `std_above_mean`parameter depending on your photodiode.  
