...
**FreelyMovingVR4mice** uses two GUIs (`vr4mice` and `dlclive-gui`) that were both developed within the Mathis lab. These two packages communicate with each other through a socket and allow for dlc poses to be passed to a video game so that the video can be rendered dynamically in real time. 
