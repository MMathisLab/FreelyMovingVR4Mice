...
class ARVisualDiscrim_single_teardrop(UnityTask):
    ...
    def __init__(self, teensy, monitor=None, write_video=False, fps=60.0, session_label = ["AR_VD_single_teardrop_blocks"],
                 ...
                 reward_size = 22, #equivalent to 5uL (basal level)
                 
                 cropped_image = [0,470,0,470],
                 #cropped_image = [100,565,30,495],
                 #cropped_image = [120,590,25,485], 
                 #cropped_image = [90,620,20,490], 

                 unity_arena_size = [-9, 9, -14, -2],
                 rotate_camera = 180, Camera_type = 1,

                 # Start_box =  [-4, 4, -11.3, -5.3, 90],

                 Start_box =  [-6, 6, -12.5, -5.5, 120],
                 L_report_box = [-9, -6.0, -5.5, -3.0], R_report_box = [6.0, 9, -5.5, -3.0],
                 # Start_box =  [-5, 5, -9.8, -2, 120], L_report_box = [-9, -6.6, -9.8, -6.8], R_report_box = [6.6, 9, -9.8, -6.8],
                 # Start_box =  [-4, 4, -8, -2, 90], L_report_box = [-9, -4, -10, -6], R_report_box = [4, 9, -10, -6],

                 Prop_Obj_on_Left = 0.5, mouse_report_delay = 0,
                 target_distance = 3,
                 distractor = 1, # yes (1), or no (0)
                 # Objects:
                 # 0 = white cube
                 # 1 = black cube
                 # 2 = teardrop grey
                 ...
                 # 7 = pacman white
                 target_selection = 7, 
                 distractor_selection = 4,
                 target_spread = 6, target_size = 1, target_height = 1,
                 slit_size = 20, slit_depth = 2, occlusion_type = 0,

                 velocity_threshold = 20, start_box_delay = 0,
                 block_length = 10, grey_screen_active = 0.0):
        
                #  reward_size = 100, cropped_image = [0,530,0,510], unity_arena_size = [-9, 9, -10, -2],
                #  R_report_box = [4, 10, -10, -8], L_report_box = [-10, -4, -10, -8], Start_box =  [-4, 4, -7, -4, 50],
                #  rotate_camera = 90,

                #  Prop_Obj_on_Left = 1, mouse_report_delay = 0.0,
        ...
        self.reward_size = [] #self.as_list(reward_size) --> changed by Luis 230829
        ...
        # Modified by Luis 230829
        self.Prob_Obj_on_Left = Prop_Obj_on_Left
        self.correctedProbObjLeft = self.Prob_Obj_on_Left
        ...
        # added by Luis 230719 (last update 230808)
        self.trialStart = 0
        self.trialEnd = 0
        self.reportedDecision = 0
        self.lastDecision = 0
        self.objectLocation = []
        self.holdDecisionStart = 0
        self.holdDecisionCurrent = 0
        self.timeInDecisionBox = 0
        self.lastDecisionStreak = 0
        self.timeBinCounter = 0
        self.binThreshold = 5
        self.correctDecisions = []
        self.decisionType = []
        self.decisionDelays = []

        # new stuff added 230829
        self.recentBiasIndex = 0
        self.correctStreak = 0
        self.rewardStreak = [22, 24, 26, 29, 31, 34]  # 21.6, 23.95, 26.35, 28.75, 31.1, 33.5 ms
        self.rewardAmount = [ 5,  6,  7,  8,  9, 10]  # uL
        self.trialReward = reward_size

        # # new stuff added 230921
        # self.soundPath = 'C:/Users/Windows/Documents/GitHub/FreelyMovingVR4Mice/'
        # self.correctSound = 'correct.wav'
        # self.incorrectSound = 'incorrect.wav'

        # # new stuff added 231012
        # self.soundTimeDelay = 0
        # self.soundStartTime = 0

        # new stuff added 231014
        self.Fs = 44100
    ...
    def _get_dlc_on_frame(self):
        ...
        # Grab information about the mouse decision (time spent in report box)
        if self.lastDecision == 0 and self.state[3] == 1:                                       # no decision has been reported yet and mouse can report (trial start)
            self.objectLocation = int(self.state[5])                                            # grab target location while present
            if self.lastDecisionStreak == 0 and self.timeBinCounter == 0 \
                and self.holdDecisionStart == 0:                                                # decision start
                if self.state [7] == 1:                                                         # in left box
                    self.holdDecisionStart = time.time()
                    self.lastDecisionStreak = self.lastDecisionStreak - 1
                    self.timeBinCounter = 1
                elif self.state[8] == 1:                                                        # in right box
                    self.holdDecisionStart = time.time()
                    self.lastDecisionStreak = self.lastDecisionStreak + 1
                    self.timeBinCounter = 1
            elif self.lastDecisionStreak != 0 and self.timeBinCounter!= 0 \
                    and self.holdDecisionStart  != 0:                                           # continues with decision
                if self.state [7] == 1:                                                         # stays in left box
                    self.holdDecisionCurrent = time.time()
                    self.lastDecisionStreak = self.lastDecisionStreak - 1
                    self.timeBinCounter = self.timeBinCounter + 1
                elif self.state[8] == 1:                                                        # stays in right box
                    self.holdDecisionCurrent = time.time()
                    self.lastDecisionStreak = self.lastDecisionStreak + 1
                    self.timeBinCounter = self.timeBinCounter + 1
                elif self.state [7] != 1 and self.state[8] != 1:                                # leaves decision box before threshold
                    self.lastDecisionStreak = 0
                    self.timeBinCounter = 0
                    self.holdDecisionStart = 0
                    self.holdDecisionCurrent = 0
        # Convert time spent in report box into a decision (once ITI has started)
        if self.lastDecision == 0 and self.state[4] == 1:                                       # inter trial interval (a decision was reported)
            if self.lastDecisionStreak <= -1*self.timeBinCounter:
                self.lastDecision = 1                                                           # left decision
            elif self.lastDecisionStreak >= self.timeBinCounter:
                self.lastDecision = 2                                                           # right decision
        # Record a decision and print session stats
        if self.lastDecision != 0 and self.reportedDecision == 0:
            self.timeInDecisionBox = 1000 * (self.holdDecisionCurrent - self.holdDecisionStart)
            self.trialEnd = time.time()
            if self.lastDecision == 1:                                                          # left decision reported                                                                      
                if self.objectLocation==1:
                    if self.correctStreak < len(self.rewardStreak):
                        self.correctStreak = self.correctStreak + 1
                    self.reward_size.insert(self.episode-1,self.rewardStreak[self.correctStreak-1])
                    self.correctDecisions.insert(self.episode-1,1)
                    self.decisionType.insert(self.episode-1,1)
                    print("               <<<<< Correct Left - Reward = ",self.rewardAmount[self.correctStreak-1],"µL >>>>>")
                    sa.play_buffer(self.correctSound, 1, 2, self.Fs)
                elif self.objectLocation==0:
                    self.correctStreak = 0
                    self.reward_size.insert(self.episode-1,0)
                    self.correctDecisions.insert(self.episode-1,0)
                    self.decisionType.insert(self.episode-1,1)
                    print("                  !!!!! Incorrect Left !!!!!")
                    sa.play_buffer(self.incorrectSound, 1, 2, self.Fs)
            elif self.lastDecision == 2:                                                        # right decision reported
                if self.objectLocation==0:
                    if self.correctStreak < len(self.rewardStreak):
                        self.correctStreak = self.correctStreak + 1
                    self.reward_size.insert(self.episode-1,self.rewardStreak[self.correctStreak-1])
                    self.correctDecisions.insert(self.episode-1,1)
                    self.decisionType.insert(self.episode-1,2)
                    print("               <<<<< Correct Right - Reward = ",self.rewardAmount[self.correctStreak-1],"µL >>>>>")
                    sa.play_buffer(self.correctSound, 1, 2, self.Fs)
                elif self.objectLocation==1:
                    self.correctStreak = 0
                    self.reward_size.insert(self.episode-1,0)
                    self.correctDecisions.insert(self.episode-1,0)
                    self.decisionType.insert(self.episode-1,2)
                    ...
                    sa.play_buffer(self.incorrectSound, 1, 2, self.Fs)
            print("     Decision delay = ",round(self.trialEnd-self.trialStart,1),"s     Time in decision box = ",round(self.timeInDecisionBox,1),"ms",sep="")
            self.decisionDelays.insert(self.episode-1,self.trialEnd-self.trialStart)
            # Overall performance
            P = self.correctDecisions.count(1)/len(self.correctDecisions)
            BI = (self.decisionType.count(2)-self.decisionType.count(1)) / len(self.decisionType)
            DD = np.mean(self.decisionDelays)
            DDstd = np.std(self.decisionDelays)
            print("     Overall: Performance = ",round(P,2),"     Bias index = ",round(BI,2),"     Delay = ",round(DD,1),"±",round(DDstd,1),"s",sep="")
            # Recent performance (last 10 trials)
            if self.episode >= 10:
                currentCorrectDecisions = self.correctDecisions[-10:]
                currentDecisionType = self.decisionType[-10:]
                currentDelay = self.decisionDelays[-10:]
                P = currentCorrectDecisions.count(1)/len(currentCorrectDecisions)
                BI = (currentDecisionType.count(2)-currentDecisionType.count(1)) / len(currentDecisionType)
                self.recentBiasIndex = BI
                DD = np.mean(currentDelay)
                DDstd = np.std(currentDelay)
                print("      Recent: Performance = ",round(P,2),"     Bias index = ",round(BI,2),"     Delay = ",round(DD,1),"±",round(DDstd,1),"s",sep="")
            self.reportedDecision = 1
    ...
    def set_channel(self):
        ...
        # Variable reward
        self.correctedProbObjLeft = self.Prob_Obj_on_Left + (self.recentBiasIndex/2)
        if self.correctStreak < len(self.rewardStreak):
            self.trialReward = self.rewardStreak[self.correctStreak]
        else:
            self.trialReward = self.rewardStreak[4]

        if self.episode == 0:
            print(" ")
            print(" ")
            print(" ")
            print("Session started",sep="")
            print(" ")
            print("Trial 1 started...","     Probability of target object on the left = ",round(self.correctedProbObjLeft,2),sep="")
        elif self.episode > 0:
            print(" ")
            print("Trial ",self.episode," started...","     Probability of target object on the left = ",round(self.correctedProbObjLeft,2),sep="")
        ...
        this_Prob_obj_left = np.random.choice([0,1],p=[1-self.correctedProbObjLeft, self.correctedProbObjLeft])
        ...
        self.channel.set_property("Object_on_Left", this_Prob_obj_left)
    ...
    def check_reward(self):
        ...
        if self.reward > 0:
            ...
            if self.state [7] > 0:
                ...
                self.teensy.write('l_water', [self.trialReward])
            else:
                #print("  <<< Rewarded - Right >>>")
                self.lastDecision = 2
                #print(self.reward_size)
                #self.teensy.write('r_water', [self.reward_size[0]])
                self.teensy.write('r_water', [self.trialReward])
