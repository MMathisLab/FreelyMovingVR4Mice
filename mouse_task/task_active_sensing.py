...
class ActiveSensingTask(UnityTask):
    ...
    def get_slit_sizes(self, slit_sizes_list: list):
        ...
        if len(slit_sizes_list) == 3:
            ...
            print(
                f"3 elements found in slit size list: using linspace function, here are your occluder sizes: {data}"
            )
        else:
            data = np.array(slit_sizes_list)
            print(
                f"{len(slit_sizes_list)} elements found, using the slit size list explicitly, here are you occluder sizes: {data}"
            )
        return data
