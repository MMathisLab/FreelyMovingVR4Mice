...
from teensyexp.tasks_abc.teensy_agent import Agent   



 # default class constructor input

class ARVisualdiscrim_socket_test(UnityTask):
    """
    ...
    def __init__(self, monitor=None, write_video=False, fps=60.0, session_label = ["AR_VD_single_teardrop_blocks"],
        ...
        super().__init__(Agent(), env_path, monitor=monitor, write_video=write_video, fps=fps, epochs=epochs, epoch_trials=True)
    ...
    def _get_dlc_on_frame(self):
        ...
        self.sent_time = this_read ["vals"][0]
        self.socket_send_time.append(self.sent_time)
        
        self.thread_read_time.append(this_read ["time"])
        if self.t_count == 0:
    ...
    def get_data(self):
        ...
        data_dict ["socket_send_time"] = np.array(self.socket_send_time)
        data_dict ["thread_read_time"] =  np.array(self.thread_read_time)
        return data_dict