...
while (time.time() - start_time) < 20:
    ...
    if this_read is not None:
        ...
        # Set the actions
        random_action = np.array([this_read ["vals"][1], this_read ["vals"][2],  0.59740335], dtype=np.float32).reshape(1,-1)
        print(random_action)
...
plt.figure()
plt.scatter(filt_sent_times[valid_times], (filt_loop_end_time[valid_times] - filt_sent_times[valid_times])*1000)
plt.xlabel("Time (s)")
