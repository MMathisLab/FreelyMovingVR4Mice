...
def process_config(config_file_path: Path) -> dict:
    ...
    keys = ["model_absolute_path", "dlc_video_absolute_path", "ar_env_unity_relative_path"]
    for k in keys:
        if k not in config_dict:
            logging.error(k + " not in " + config_file_path)

    # Check if the paths specified in the config file exist on the file system
    ar_path = Path(config_file_path).parent
    ar_env_path = Path(ar_path).joinpath(config_dict["ar_env_unity_relative_path"])
    normalized_path = ar_env_path.resolve()
    config_dict["ar_env_unity_relative_path"] = str(normalized_path)
