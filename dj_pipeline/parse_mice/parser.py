...
def parse_xlsx_exp(file_path, sheet_name):
    ...
    for i, row in enumerate(sheet.iter_rows(values_only=True)):
        ...
        if i ==  0:
            ...
            row_dict = {headers[j]: row[j] for j in range(len(row)) if row[j] is not None}
...
def save_row_as_npy(exp, mice_info, i, mouse_name=None):
    ...
    try:
        ...
        sex = 'F' if mice_info['sex'] == 'Female' else 'M' if mice_info['sex'] == 'Male' else mice_info['sex']
        weight_percentage = round((float(exp['weight']) - float(mice_info['baseline_weight'])) / float(mice_info['baseline_weight']) * 100, 2)
        ...
        np.save(f"{output['mouse_name']}_{output['doe']}_{output['attempt']}.npy", output)
