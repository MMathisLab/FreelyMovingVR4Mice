...
class Opto(Template):
    ...
    def __init__(self, dj_dict, widget):
        super().__init__(
            widget=widget,
            nick="opto",
            labels=_set_opto_labels(),
            choices=_set_opto_choices(dj_dict),
        ...
        for key, value in labels.items():
            ...
            if key in self.special_fields.keys():
                ...
                self._create_special_field(
                    key=key, labels=labels, layout=layout, i=i, j=0
                )
            else:
    ...
    def _selected_opto_callback(self, evt=None):
        ...
        if value.lower() == "none":
            for key, value in self.values.items():
                all_items = [
                    self.values[key].itemText(i)
                    for i in range(self.values[key].count())
                ]
    ...
    def _create_button(self, key):
        ...
        button.clicked.connect(lambda evt, args=key: self._activated(evt, key))
        return button
