...
def load_data(
        path:
    str = "/Users/thomassainsbury/Documents/Mathis_lab/Aug_Reg/AR_example_data/",
    ...
    state_dict = pd.read_pickle(path + mouse_name + "_" + date + "_" +
                                attempt + ".pickle")
    ...
    df = pd.DataFrame({
        ...
        state_dict["state"][:, 1],
        "aperture":
        state_dict['slit_size'][state_dict['episode'] - 1],
        "head_dir":
        state_dict["state"][:, 2],
        "mouse_can_report":
        state_dict["state"][:, 3],
    ...
    df = df[
        df.trial !=
        1]  #NOTE(celia): drop first trial which is DLC-live initialization trial

    df["x"] = np.interp(df.x, [-9, 9], [-27, 27])
    df["y"] = np.interp(df.y, [-10, -2], [-27, 27])

    df["bins_y"] = pd.cut(df["y"],
                          bins=np.linspace(spatial_ybins[0], spatial_ybins[1],
                                           spatial_ybins[2]))
    df["bins_x"] = pd.cut(df["x"],
                          bins=np.linspace(spatial_ybins[0], spatial_ybins[1],
                                           spatial_ybins[2]))
    ...
    df["trial_rewarded"] = df.groupby(
        "trial", as_index=False)["reward"].transform(lambda x: x.max())
    df[["trial_step", "trial_step_time"]] = df.groupby(
        "trial", as_index=True,
        group_keys=False)[["step",
                           "step_time"]].apply(lambda x: x.iloc[:] - x.iloc[0])

    if no_iti == True:
        df = df[df.iti == 0.0]
        df["trial_step_fraction"] = df.groupby(
            "trial", as_index=True, group_keys=False)["trial_step"].apply(
                lambda x: x.iloc[:] / x.iloc[-1])
        df["trial_R_choice"] = df.groupby(
            "trial",
            as_index=False)["mouse_in_R"].transform(lambda x: x.iloc[-1])
        df["trial_L_choice"] = df.groupby(
            "trial",
            as_index=False)["mouse_in_L"].transform(lambda x: x.iloc[-1])
    else:
        df["trial_step_fraction"] = df.groupby(
            "trial", as_index=True, group_keys=False)["trial_step"].apply(
                lambda x: x.iloc[:] / x.iloc[-1])
    ...
    categorical_columns = [
        "mouse_in_R", "mouse_in_L", "aperture", "reward", "iti"
    ]
    continuous_columns = df.columns[~df.columns.isin(categorical_columns)]

    df['time'] = pd.to_datetime(df['step_time'], unit='s')
    categorical_resampled = df.set_index("time").groupby(
        'trial',
        as_index=False)[categorical_columns].resample('0.02s').first()
    continuous_resampled = df.set_index("time").groupby(
        'trial', as_index=False)[continuous_columns].resample(
            '0.02s').mean().interpolate()
    df = pd.concat([continuous_resampled, categorical_resampled],
                   axis=1).reset_index()
    ...
    df["velocity"] = np.sqrt((np.gradient(df.x, df.time_elapsed)**2) +
                             (np.gradient(df.y, df.time_elapsed)**2))

    df["angular_velocity"] = np.gradient(df.head_dir, df.time_elapsed)
    df['angular_acceleration'] = np.gradient(df['angular_velocity'],
                                             df.time_elapsed)
    ...
    df["trial_length"] = df.groupby(
        "trial", as_index=False)["time_elapsed"].transform(
            lambda x: x.iloc[-1] - np.mean(x.iloc[:5]))
    df["trial_traj_path_length"] = df.groupby(
        "trial", as_index=False)["velocity"].transform("sum")
    df["trial_init_x"] = df.groupby(
        "trial", as_index=False)["x"].transform(lambda x: x.iloc[0])
    df["trial_init_y"] = df.groupby(
        "trial", as_index=False)["y"].transform(lambda y: y.iloc[0])
    df["trial_end_x"] = df.groupby(
        "trial", as_index=False)["x"].transform(lambda x: x.iloc[-1])
    df["trial_end_y"] = df.groupby(
        "trial", as_index=False)["y"].transform(lambda y: y.iloc[-1])
    ...
    df["session"] = df['mouse_name'].astype(str) + '_' + df['date'].astype(
        str) + '_' + df['attempt'].astype(str)
    ...
    box_df["left_reward_x"] = df[(df.reward > 0.5)
                                 & (df.trial_L_choice > 0.5)]["x"].mean()
    box_df["left_reward_z"] = df[(df.reward > 0.5)
                                 & (df.trial_L_choice > 0.5)]["y"].mean()
    box_df["right_reward_x"] = df[(df.reward > 0.5)
                                  & (df.trial_R_choice > 0.5)]["x"].mean()
    box_df["right_reward_z"] = df[(df.reward > 0.5)
                                  & (df.trial_R_choice > 0.5)]["y"].mean()
    ...
    starting_positions['bin_idx'] = np.digitize(
        starting_positions['x'], bin_edges,
        right=False) - 1  # Adjust bin index to be 0-based
    starting_positions['x_init_bin_center'] = starting_positions[
        'bin_idx'].apply(lambda x: bin_midpoints[x]
                         if x < len(bin_midpoints) else np.nan)

    df = pd.merge(df,
                  starting_positions[['trial', 'x_init_bin_center']],
                  on='trial',
                  how='left')
...
def load_and_sync_dlc_w_game(mouse_name, date, attempt, path, game_data):
    ...
    dlc_dict = dlc_helpers.load_dlc_proc(mouse_name=mouse_name,
                                         date=date,
                                         attempt=attempt,
                                         path=path)
    ...
    df_out["head_angle_velocity"] = np.gradient(
        df_out.head_angle, df_out.time_elapsed)  #df_out.head_angle.diff()
    df_out["heading_dir_velocity"] = np.gradient(
        df_out.heading_dir, df_out.time_elapsed)  #df_out.heading_dir.diff()

    df_out["head_angle_acceleration"] = np.gradient(
        df_out.head_angle_velocity,
        df_out.time_elapsed)  #df_out.head_angle.diff()
    df_out["heading_dir_acceleration"] = np.gradient(
        df_out.heading_dir_velocity,
        df_out.time_elapsed)  #df_out.heading_dir.diff()
...
def _define_box(box_df, state_dict, which):
    ...
    box_df[f"{which}_box_x_center"] = (box_df[f"{which}_box_x_min"] +
                                       box_df[f"{which}_box_x_max"]) / 2
    box_df[f"{which}_box_z_center"] = (box_df[f"{which}_box_z_min"] +
                                       box_df[f"{which}_box_z_max"]) / 2
...
def get_all_tolias_mice(mouse_list, path, load_dlc=True):
    ...
    big_df = []
    for m in mouse_list:
        df, box_df = load_data(path=path,
                               mouse_name=m["mouse_name"],
                               date=m["date"],
                               attempt=m["attempt"])
        if load_dlc == True:
            df = load_and_sync_dlc_w_game(path=path,
                                          mouse_name=m["mouse_name"],
                                          date=m["date"],
                                          attempt=m["attempt"],
                                          game_data=df)
...
def create_bins(data, spatial_ybins=[-13, 24, 50]):
    ...
    """
    data["bins"] = pd.cut(data["y"],
                          bins=np.linspace(spatial_ybins[0], spatial_ybins[1],
                                           spatial_ybins[2]))
    data["bin_centers"] = data["bins"].apply(lambda x: x.mid).astype(
        "float") - 25
    return data


def get_spatial_normalisation_params(data, spatial_ybins=[-13, 24, 50]):
    data["norm_head_dir"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["head_dir"].transform(
            lambda x: x - np.mean(x.iloc[:5]))
    data["trial_length"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["time_elapsed"].transform(
            lambda x: x.iloc[-1] - np.mean(x.iloc[:5]))
    data["trial_traj_path_length"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["velocity"].transform("sum")
    data["trial_init_x"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["x"].transform(lambda x: x.iloc[0])
    data["trial_init_y"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["y"].transform(lambda y: y.iloc[0])
    data["trial_end_x"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["x"].transform(lambda x: x.iloc[-1])
    data["trial_end_y"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["y"].transform(lambda y: y.iloc[-1])
    data["trial_direct_path"] = np.sqrt(
        (((data.trial_init_x - data.trial_end_x)**2) +
         (data.trial_init_y - data.trial_end_y)**2))
    data[
        "trial_tortuosity"] = data.trial_traj_path_length / data.trial_direct_path
    data["bins"] = pd.cut(data["y"],
                          bins=np.linspace(spatial_ybins[0], spatial_ybins[1],
                                           spatial_ybins[2]))
    data["norm_y"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["y"].transform(lambda x: x - x.iloc[0])
    data["norm_x"] = data.groupby(
        ["mouse_name", "date", "attempt", "trial"],
        as_index=False)["x"].transform(lambda x: x - x.iloc[0])
    data["bin_centres"] = data["bins"].apply(lambda x: x.mid).astype(
        "float") - 25
    return data
...
def interpolate(
        df,
        n_points=100,
        interpolation_columns=["mouse_name", "date", "attempt", "trial"],
    ...
    interpolated_dfs = [
        interpolate_group(group, n_points, interpolation_columns,
                          value_columns)
...
def calculate_choice_bin(df, trial_rewarded=0.5, trial_tortuosity_thresh=100):
    mean_mice = df.groupby([
        "mouse_name", "date", "attempt", "aperture", "trial_L_choice",
        "bin_centres"
    ],
                           as_index=False).mean(numeric_only=True)
    mean_mice["over_bound"] = (abs(mean_mice.norm_x) > 5).diff() > 0
    mean_mice["y_over_bound"] = mean_mice["bin_centres"][
        mean_mice["over_bound"]]
    mean_mice = mean_mice[(mean_mice["bin_centres"] > -20)
                          & (mean_mice["bin_centres"] < -5)]
    mean_mice = mean_mice.dropna().copy()
    mean_mice = mean_mice.groupby(
        ["mouse_name", "date", "attempt", "aperture"], as_index=False).last()
    plt.figure(figsize=(3, 6))
    sns.lineplot(data=mean_mice,
                 x="aperture",
                 y="y_over_bound",
                 estimator=None,
                 ...
                 color="black",
                 alpha=0.5)
    sns.scatterplot(data=mean_mice,
                    x="aperture",
                    y="y_over_bound",
                    color="black")
    plt.xlim(0, 15)
...
def plot_choice_per_mouse(df, mouse_list):
    ...
    for i in range(len(mouse_list)):
        ...
        ax[i].set_title(
            str(mouse.mouse_name.iloc[0]) + "_" + str(mouse.date.iloc[0]))
