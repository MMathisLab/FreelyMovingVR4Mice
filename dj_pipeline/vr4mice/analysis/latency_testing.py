...
def check_data(data):
    ...
    vars = ["frame_time", "frame_time", "timestamp", "photodiode_read", "photodiode_time", "signal"]
    for var in vars:
        if var not in data:
    ...
    photodiode_read =  data["photodiode_read"] * detect_signal_polarity(data["photodiode_read"])
...
def has_signal(data, threshold_sigma=8.0):
    ...
    try:
        delay = data["frame_time"][np.where(data["signal"] > 0.5)[0][0]] - data["start_time"]
    except:
        return False

    # check if peak signal is above noise threshold
    min_start = np.mean(signal [(photodiode_time > delay-3) & (photodiode_time < delay-0.5)])
    std = np.std(signal [(photodiode_time > delay-3) & (photodiode_time < delay-0.5)])
