    ...
    for i in range(dlc_steps.size):
        ...
        while not found_step:
            if step_time_index == len(step_time) - 1:
                ...
                next_distance = np.abs(step_time[step_time_index + 1] -
                                       dlc_times[i])
...
def load_dlc_proc(
    ...
    dlc_dict = pd.read_hdf(path + camera_name + "_" + mouse_name + "_" + date +
                           "_" + attempt + "_DLC.hdf5")
...
def filter_dlc(dlc_dict, cutoff=0.4, window_length=9, polyorder=3):
    ...
    for b in body_parts:
        ...
        trace_x = dlc_interpolate(key_point_x,
                                  likelyhood=likelihood,
                                  cutoff=cutoff)
        trace_y = dlc_interpolate(key_point_y,
                                  likelyhood=likelihood,
                                  cutoff=cutoff)
...
def getall_dlc_heading_angles(filt_dlc):
    ...
    for i in range(filt_dlc.shape[0]):
        x, y, heading_angle, head_angle, body_axis, head_axis = compute_dlc_heading_angles(
            filt_dlc.iloc[i][:-2])
...
def find_closest_indices(pose_time, step_time):
    ...
    for step in step_time:
        ...
        if idx == 0:
            ...
            after = pose_time[idx]
            closest_indices.append(idx if step - before > after -
                                   step else idx - 1)
