...
def plot_on_subplot(ax, data_dict, frame_number, heading_direction):
    ...
    body_parts = data_dict.columns.get_level_values(
        0).unique()[:-2]  # Example body parts
    for b in body_parts:
        ...
        likelihood = data_dict[b]['likelihood'][
            frame_number]  # Example of accessing likelihood
        ax.scatter(x, y, alpha=likelihood, c=likelihood, vmin=0,
                   vmax=1)  # Plot with alpha based on likelihood
...
def make_animated_movie(
    ...
    orig = dlc_data.copy()
    filt_dlc = filter_dlc(dlc_data.copy(),
                          cutoff=cutoff,
...
def plot_dlc_heading_direction(ax, df, computed_dlc, frame):
    ...
    ax.quiver(computed_row.head_center_x,
