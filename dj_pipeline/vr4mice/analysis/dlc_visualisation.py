...
def plot_on_subplot(ax, data_dict, frame_number, heading_direction):
    ...
    body_parts = data_dict.columns.get_level_values(0).unique()[:-2]  # Example body parts
    for b in body_parts:
        ...
        likelihood = data_dict[b]['likelihood'][frame_number]  # Example of accessing likelihood
        ax.scatter(x, y, alpha=likelihood, c=likelihood, vmin = 0, vmax=1)  # Plot with alpha based on likelihood
    ...
    ax.arrow(mean_x, mean_y, heading_direction[0], heading_direction[1], 
             width=1, color='red', length_includes_head=True)
    x = data_dict[b]['x'][frame_number]
    y = data_dict[b]['y'][frame_number]
    ax.set_xlim(0, 550)
    ax.set_ylim(0, 550)
...
def make_animated_movie(dlc_data, frames = [1,500,10], cutoff = 0.3, window_length = 9, polyorder = 3, path=  "/Users/thomassainsbury/Documents/Mathis_lab/Aug_Reg/AR_animation.mp4"):
    # Initialize figure and axes
    fig, axs = plt.subplots(1, 2, figsize=(10, 5))

    orig = dlc_data.copy()
    filt_dlc = filter_dlc(dlc_data.copy(),  cutoff=cutoff, window_length=window_length, polyorder=polyorder)
    # Setup animation
    ani = FuncAnimation(fig, update_plots, frames=range(frames [0], frames [1],frames [2]), fargs=(filt_dlc, orig, axs), interval=10)
...
def plot_dlc_heading_direction(ax, df, computed_dlc, frame):
    ...
    ax.scatter(keypoints[:, 0], keypoints[:, 1], c=likelihood, alpha=0.5, vmin=0, vmax=1)

    # Plot the heading direction as an arrow
    ax.quiver(computed_row.head_center_x, computed_row.head_center_y, u, v, color="red", scale=15, width=0.005)
    ax.quiver(computed_row.head_center_x, computed_row.head_center_y, uh, vh, color="green", scale=15, width=0.005)
    ax.set_xlim(0,600)
    ax.set_ylim(0,700)
...
def create_animation_dlc_w_heading_angle(df, computed_dlc, frames):
    ...
    ani = FuncAnimation(fig, update, frames=frames, fargs=(ax, df, computed_dlc), interval=50)
