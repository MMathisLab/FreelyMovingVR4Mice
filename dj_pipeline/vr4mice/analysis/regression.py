...
def find_decision_point_proba(df, threshold_uncertainty=0.3):
    ...
    """
    decision_point = df.groupby(["session", "trial"], as_index=False).apply(lambda x: find_decision_point_per_trial(x, threshold_uncertainty))
...
def find_decision_point_per_trial(trial_data, threshold_uncertainty):
    ...
    for index in above_threshold.index:
        ...
        if all(trial_data["trial_R_choice"] > 0.5) and all(subsequent_values >= above_threshold.loc[index, 'pred']):
            # Returning the step of the decision point
            return trial_data.loc[index]
        elif all(trial_data["trial_R_choice"] < 0.5) and all(subsequent_values <= above_threshold.loc[index, 'pred']):
            return trial_data.loc[index]
    ...
    for session_id, session in enumerate(df.session.unique()):
        for trial_id, trial in df[df["session"]==session].groupby(["trial"]):
            ...
            # Reset index within each group for a uniform timescale
                ...
                sns.lineplot(data=trial, x="bin_centers", y="pred", ax=ax[session_id], errorbar="se")
...
def plot_decision_points_on_trajectory(df, df_box, decision_point, color="red", session="30559_2024-02-19_1", trials=list(range(25, 30)), ax=None):
    """
    ...
    for trial in df[df["session"]==session].trial.unique():
        if trial in trials:
        #if trial == 26:
            ax.plot(df[(df["session"]==session) & (df["trial"]==trial)]["x"], df[(df["session"]==session) & (df["trial"]==trial)]["y"])
            mpl.rcParams['lines.markersize'] = 10
            ax.scatter(decision_point[(decision_point["session"]==session) & (decision_point["trial"]==trial)]["x"],
                        decision_point[(decision_point["session"]==session) & (decision_point["trial"]==trial)]["y"], color=color)
...
def find_decision_point_from_distance(trial_data, df_box):
    ...
    if all(trial_data["trial_R_choice"] > 0.5):
        ...
        trial_data["dist"] = np.sqrt((df_box["right_reward_x"] - trial_data["x"])**2 + (df_box["right_reward_z"] - trial_data["y"])**2)
        
    else:
        trial_data = trial_data[trial_data["mouse_in_L"]<1]
        #trial_data["dist"] = abs(df_box["left_reward_x"] - trial_data["x"])
        trial_data["dist"] = np.sqrt((df_box["left_reward_x"] - trial_data["x"])**2 + (df_box["left_reward_z"] - trial_data["y"])**2)
...
def pair_plot(decision_point, ax=None): 
    ...
    mean_mice = decision_point.groupby(["session", "aperture"], as_index=False).mean(numeric_only=True)
    
    sns.lineplot(ax=ax, data=mean_mice, x="aperture", y=np.abs(mean_mice.y)-25, estimator =None, units=mean_mice.session, sort=False, color="black", alpha=0.5)
    sns.scatterplot(ax=ax, data=mean_mice, x="aperture", y=np.abs(mean_mice.y)-25, color="black")
    ax.set_xlim(3,13)
    #plt.ylim(5,20)
    ax.set_ylim(0,-25)
    ax.set_ylabel("Decision distance to screen (cm)")
    ax.set_xlabel("Occluder")
    print(stats.ttest_rel(mean_mice[mean_mice["aperture"]==4.3]["y"], mean_mice[mean_mice["aperture"]==12]["y"]) )
    #plt.savefig("seaborn_plot.svg")