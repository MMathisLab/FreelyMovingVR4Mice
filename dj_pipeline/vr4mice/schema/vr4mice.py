...
class SignalsPhotodiode(dj.Computed):
    ...
    signal_type            : varchar(32)      # type of signal generated (e.g., 'pulse', 'pulse_geo')
    signal_delay           : int              # delay between signal generation and photodiode response
    """
    def make(self, key):
        from vr4mice.actions.populate_rig import get_files_paths
            ...
            if os.path.exists(proc_filepath):
                ...
                if check_data(photodiode_data):
                    data = {
                        ...
                        "signal_type": photodiode_data["signal_type"],
                        "signal_delay": photodiode_data["signal_delay"],
                    }
