...
def map_video_frames_to_step_time(sync_df, time_dict):
    ...
    if 'dlc_read_time' not in time_dict or 'step_time' not in time_dict:
        ...
        raise ValueError(f"Dictionary must contain 'dlc_read_time' and 'step_time' keys. Available keys: {available_keys}")
    ...
    print(f"Sync send time range: {sync_send_times[0]:.3f}s to {sync_send_times[-1]:.3f}s")
    print(f"Dict DLC read time range: {dict_dlc_times[0]:.3f}s to {dict_dlc_times[-1]:.3f}s")
    print(f"Dict step time range: {dict_step_times[0]:.3f}s to {dict_step_times[-1]:.3f}s")
    
    # Calculate sampling rates
    sync_send_rate = 1.0 / np.mean(np.diff(sync_send_times))
    dict_dlc_rate = 1.0 / np.mean(np.diff(dict_dlc_times)) if len(dict_dlc_times) > 1 else 0
    step_rate = 1.0 / np.mean(np.diff(dict_step_times)) if len(dict_step_times) > 1 else 0
    ...
    for i, step_time in enumerate(dict_step_times):
        ...
        if len(dict_dlc_times) > 1:
            ...
            if dlc_index_floor == dlc_index_ceil:
                ...
                corresponding_dlc_time = (dict_dlc_times[dlc_index_floor] * (1 - weight) + 
        ...
        time_tolerance = 1.0 / step_rate if step_rate > 0 else 0.1  # Use step sampling period as tolerance
...
def main():
    ...
    try:
        ...
        if 'video_frame_index' not in sync_df.columns or 'send_time' not in sync_df.columns:
            # Try to find similar column names
            possible_frame_cols = [col for col in sync_df.columns if 'frame' in col.lower()]
            possible_time_cols = [col for col in sync_df.columns if 'send' in col.lower() and 'time' in col.lower()]
            if not possible_time_cols:
                possible_time_cols = [col for col in sync_df.columns if 'time' in col.lower()]
            
            if possible_frame_cols and possible_time_cols:
                print(f"  Auto-detecting columns: frame='{possible_frame_cols[0]}', time='{possible_time_cols[0]}'")
                ...
                raise ValueError("Could not find video_frame_index and send_time columns")
    ...
    try:
        if time_dict_file_path.endswith('.pkl') or time_dict_file_path.endswith('.pickle'):
        ...
        if 'dlc_read_time' in time_dict and 'step_time' in time_dict:
            ...
            available_keys = list(time_dict.keys())
            print(f"  Error: Expected 'dlc_read_time' and 'step_time' keys, found: {available_keys}")
            return
